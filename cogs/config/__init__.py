...
EVENTS_DESCRIPTIONS = {
    ...
    "on_member_ban": "Triggered when a member is banned from the server. This event also triggers Member Leave event.",
    "on_member_unban": "Triggered when a member is unbanned from the server.",
    "on_message_delete": "Triggered when a message is deleted. Bot message deletion is not tracked.",
    "on_message_edit": "Triggered when a message is edited. Eg. when content of a message is changed. Bot message edits are not tracked.",
    "on_channel_delete": "Triggered when a channel is deleted.",
    "on_channel_create": "Triggered when a channel is created.",
    "on_channel_update": "Triggered when a channel is updated. Eg. when channel name is changed or channel topic is changed.",
    "on_thread_create": "Triggered when a thread is created.",
    "on_thread_delete": "Triggered when a thread is deleted.",
    "on_thread_update": "Triggered when a thread is updated. Eg. when thread name is changed or thread archived status is changed.",
    "on_server_update": "Triggered when the server is updated. Eg. when server name is changed or server icon is changed.",
    "on_role_create": "Triggered when a role is created.",
    "on_role_delete": "Triggered when a role is deleted.",
    "on_role_update": "Triggered when a role is updated. Eg. when role name is changed or role permissions are changed.",
    "on_member_join_voice": "Triggered when a member joins a voice channel. Bot voice channel joins are not tracked.",
    "on_member_leave_voice": "Triggered when a member leaves a voice channel. Bot voice channel leaves are not tracked.",
    "on_member_move_voice": "Triggered when a member moves between voice channels. Bot voice channel moves are not tracked.",
}
...
LEVELING_REWARD_ROLES_DESCRIPTION = """
        ...
        view = PaginationLayout(
            ...
            items=[
                [
                    ...
                    discord.ui.ActionRow(HubChannelSelect(channel_id=config["hub_channel_id"])),
                    discord.ui.Separator(),
                    discord.ui.TextDisplay(BIRTHDAY_CHANNEL_DESCRIPTION),
                    discord.ui.ActionRow(BirthdayChannelSelect(channel_id=config["birthday_config"]["channel_id"])),
                    discord.ui.Separator(),
                    discord.ui.TextDisplay(TELEPHONE_CHANNEL_DESCRIPTION),
                    discord.ui.ActionRow(TelephoneChannelSelect(channel_id=config["telephone_config"]["channel_id"])),
                    discord.ui.Separator(),
