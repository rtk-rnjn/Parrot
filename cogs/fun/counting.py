...
class Counting(Cog):
    ...
    async def get_last_message(self, channel: discord.TextChannel) -> typing.Optional[discord.Message]:
    ...
    async def on_message(self, message: discord.Message):
        ...
        if msg:
            if message.author.id == msg.author.id:
                try:
                    return await message.delete(reason=f"Can't post more than once in a row")
                except Exception:
        ...
        try:
            ...
            try:
                await message.delete(reason=f"Message the {message.author} send wasn't a number")
            except Exception:
                return await message.channel.send(f"Bot need manage message permission to work properly")
        
        if new_number - 1 != number:
            try:
                return await message.delete(reason=f"Invalid count in Counting channel")
            except Exception:
                return await message.channel.send(f"Bot need manage message permission to work properly")
