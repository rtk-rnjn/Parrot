...
def create_snek_frame(  # noqa: PLR0917
...
class SnakeAndLaddersGame:
    ...
    async def open_game(self) -> None:
        ...
        startup = await self.channel.send(
            f"Press {JOIN_EMOJI} to participate, and press {START_EMOJI} to start the game",
        )
        ...
        while not self.started:
            try:
                reaction, user = await self.ctx.bot.wait_for("reaction_add", timeout=300, check=startup_event_check)
