...
class Ship:
    def __init__(
        ...
        self.end: Coords = (self.start[0], self.start[1] + self.size - 1) if self.vertical else (self.start[0] + self.size - 1, self.start[1])
...
class Board:
    ...
    def to_image(self, hide: bool = False) -> BytesIO:
        ...
        with Image.open("assets/battleship.png") as img:
            ...
            for i, y in zip(range(1, 11), range(75, 530, 50), strict=True):
                for j, x in zip(range(1, 11), range(75, 530, 50), strict=True):
                    coord = (i, j)
...
class CoordButton(discord.ui.Button["BattleshipView"]):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        view = self.view.initialize_view(clear=True)
...
class BattleshipView(BaseView):
    ...
    def initialize_view(self, *, clear: bool = False, start: bool = False) -> Self:
...
class SetupView(BaseView):
    def __init__(self, game: BetaBattleShip, timeout: float | None) -> None:
        super().__init__(timeout=timeout)
