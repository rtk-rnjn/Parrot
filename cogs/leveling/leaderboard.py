...
class Leveling(commands.Cog, name="Leveling"):
    ...
    async def rank(self, ctx, target: discord.Member = None):
        ...
        for current_guild in level['levels']:
            if (current_guild['guild_id']
                    == ctx.guild.id) and (current_guild['name'] == target.id):
                xp = current_guild['xp']
                lvl = current_guild['lvl']
                await ctx.send(embed=discord.Embed(
                    title=f"{target.name}",
                    description=
                    f"`{target.name}` Chat experience is **{xp}** and on level **{int(lvl / 100)}**! GG!",
            ...
            await ctx.send(embed=discord.Embed(
                title=f"{target.name}",
                description=
                f"`{target.name}` Chat experience is **10** and on level **0**! GG!"
            ),
                           timestamp=datetime.datetime.utcnow())
    ...
    async def disableleveling(self, ctx):
        ...
        if ctx.guild.id in guild['guilds']:
            return await ctx.send(
                f"{ctx.author.mention} `leveling system` already disabled!")
        ...
        await ctx.reply(
            f"{ctx.author.mention} Successfully disabled `leveling system` in this server."
        )
    ...
    async def enableleveling(self, ctx):
        ...
        if ctx.guild.id not in guild['guilds']:
            return await ctx.send(
                f"{ctx.author.mention} `leveling system` already enabled!")
        ...
        await ctx.reply(
            f"{ctx.author.mention} Successfully enabled `leveling system` in this server."
        )
