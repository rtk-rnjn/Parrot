    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            def soup_match(tag):
                return all(
                    string in tag.text
                    for string in text.strip().split()) and tag.name == 'li'
            ...
            emb.add_field(name=f'Results for `{text}` :',
                          value='\n'.join(content),
                          inline=False)
...
async def haskell_doc(ctx, text: str):
    ...
    async with aiohttp.ClientSession() as client_session:
        async with client_session.get(url) as response:
            ...
            results = BeautifulSoup(await response.text(),
                                    'lxml').find('div', class_='searchresults')

            if results.find('p',
                            class_='mw-search-nonefound') or not results.find(
