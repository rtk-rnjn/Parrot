...
class AtCoder:
    ...
    async def fetch(self, http_session: aiohttp.ClientSession | None = None):
    ...
    async def _get_contests(self, http_session: aiohttp.ClientSession | None = None):
        if http_session is None:
            async with aiohttp.ClientSession() as session:
                return await self._get_contests(session)
        else:
            soup = await self.fetch(http_session)
