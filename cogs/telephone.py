...
class Telephone(Cog, name='telephone'):
    ...
    async def telsetup(self,
        ...
        if not arg or not settings:
            ...
            await ctx.send(
                f"Invalid setting. Available setting type: `channel`, `pingrole`, `memberping`, `block` `unblock. This server current Telephone Settings are:-\n"
                f"> Telephone Channel:\n`{data['channel']}`\n"
        ...
        if (setting.lower() in settings) and (type(arg) in [
                discord.TextChannel, discord.Role, discord.Member
        ]):
            await telephone_update(ctx.guild.id, setting.lower(), arg.id)
            await ctx.reply(
                f'{ctx.author.mention} Success! {setting.lower()}: {arg.name} ({arg.id})'
            )
