...
async def _new(ctx: Context, args: str):
    ...
    await collection.update_one({'_id': ctx.guild.id}, {'$addToSet': {'ticket_channel_ids': ticket_channel.id}})
    ...
    if data['log']:
        ...
        if log_channel:
            await log(ctx.guild, 
                    log_channel, f"**Channel:** #ticket-{data['ticket_number']+1}\n"
    ...
    if data := await collection.find_one({'_id': ctx.guild.id}):
        if ctx.channel.id in data["ticket_channel_ids"]:
            ...
            try:
                ...
                await collection.update_one({"_id": ctx.guild.id}, {'$pull': {'ticket_channel_ids': ctx.channel.id}})
                if data['log']:
                    log_channel = ctx.guild.get_channel(data['log'])
                    if log_channel:
                        await log(ctx.guild, 
