...
class Utils(Cog):
    ...
    async def get_all_timers(self,
                             member: discord.User) -> list[discord.Embed]:
        collection = self.collection
        records = []
        async for data in collection.find({'author': member.id}):
            embed = discord.Embed(
                ...
                color=member.color).set_footer(text=f'{member}')
    ...
    @commands.command()
    @Context.with_type
    async def remindme(self, ctx: Context, age: ShortTime, *,
                       task: commands.clean_content):
        ...
        await self.create_timer(ctx.channel,
                                ctx.message,
                                ctx.author,
                                seconds,
                                remark=task,
    ...
    async def reminders(self, ctx: Context):
        ...
        if not em_list:
            return await ctx.reply(
                f"{ctx.author.mention} you don't have any reminders")
    ...
    @commands.command()
    @Context.with_type
    async def remindmedm(self, ctx: Context, age: ShortTime, *,
                         task: commands.clean_content):
    ...
    async def tag(self, ctx: Context, *, tag: str = None):
        ...
        if not ctx.invoked_subcommand and tag is not None:
            await mt._show_tag(
                self.bot, ctx, tag, ctx.message.reference.resolved
    ...
    @commands.bot_has_permissions(embed_links=True)
    async def tag_tranfer(self, ctx: Context, tag: str, *,
                          member: discord.Member):
    ...
    @commands.bot_has_permissions(embed_links=True)
    async def settime_todo(self, ctx: Context, name: str, *,
                           deadline: ShortTime):
        ...
        async for data in giveaway.find(
            ...
            for reaction in msg.reactions:
                if str(reaction) == "\N{PARTY POPPER}":
                    ...
                    ls = await mt.get_winners(users=users,
