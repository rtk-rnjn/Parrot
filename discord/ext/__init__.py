...
class Position:
    ...
    def __hash__(self):
        return hash((self.number, self.bucket))
...
class Button:
    ...
    def __init__(self, emoji, action, *, skip_if=None, position: Position | None = None, lock=True):
    ...
    def skip_if(self, value):
        ...
        try:
            ...
            if not isinstance(menu_self, Menu):
                error_message = f"skip_if bound method must be from Menu not {menu_self!r}"
    ...
    def action(self, value):
        try:
            ...
            # Unfurl the method to not be bound
            if not isinstance(menu_self, Menu):
                error_message = f"action bound method must be from Menu not {menu_self!r}"
                raise TypeError(error_message)

            value = value.__func__

        if not inspect.iscoroutinefunction(value):
            error_message = f"action must be a coroutine not {value!r}"
            raise TypeError(error_message)
        ...
        if inherit_buttons:
            ...
            for base in reversed(new_cls.__mro__):
                for value in base.__dict__.values():
                    if getattr(value, "__menu_button__", False):
...
class Menu(metaclass=_MenuMeta):
    ...
    def _verify_permissions(self, ctx: commands.Context, channel: discord.TextChannel, permissions: discord.Permissions):
    ...
    def reaction_check(self, payload: discord.RawReactionActionEvent):
        ...
        assert self.bot is not None and self.message is not None

        if payload.message_id != self.message.id:
            return False
        if payload.user_id not in {self.bot.owner_id, self._author_id, *(self.bot.owner_ids or set())}:
            return False

        return payload.emoji in self.buttons

    async def _internal_loop(self):  # noqa: C901, PLR0912
        tasks = []
        ...
        try:
            ...
            assert self.bot and self.message
...
class PageSource:
    ...
    async def _prepare_once(self):
        if not hasattr(self, "_PageSource__prepare"):
            await self.prepare()
            self.__prepare = True
...
class MenuPages(Menu):
    ...
    async def change_source(self, source):
        ...
        if not isinstance(source, PageSource):
            error_message = f"Expected {PageSource!r} not {source.__class__!r}."
            raise TypeError(error_message)
    ...
    async def show_page(self, page_number):
        ...
        assert self.message is not None and kwargs
    ...
    async def start(self, ctx: commands.Context, *, channel: discord.abc.Messageable | None = None, wait=False):
    ...
    async def go_to_last_page(self, payload):
        ...
        await self.show_page((self._source.get_max_pages() or 1) - 1)
...
class GroupByPageSource(ListPageSource):
    ...
    def __init__(self, entries, *, key, per_page, sort=True):
        ...
        for group_key, group in itertools.groupby(self.__entries, key=key):
            items = list(group)
            if not items:
                continue
            size = len(items)
            ...
            nested.extend(_GroupByEntry(key=group_key, items=items[i : i + per_page]) for i in range(0, size, per_page))
...
def _aiter(obj, *, _isasync=inspect.iscoroutinefunction):
    ...
    try:
        ...
        message = f"{cls.__name__!r} object is not an async iterable"
