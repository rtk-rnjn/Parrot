...
class AkiView(discord.ui.View):
    ...
    def __init__(self, *, bot: Parrot, ctx: Context, game: Akinator, player: discord.Member, q: str):
    ...
    async def is_game_ended(self) -> None:
        if self.game.progression <= 80:
            ...
            embed = discord.Embed(
                ...
                color=self.bot.color, timestamp=self.message.created_at
            )
            ...
            if val is None:
                return
            if val:
                await self.message.channel.send(f"{self.message.author.mention} Yayy! Guessed right huh!")
            else:
                await self.message.channel.send(f"{self.message.author.mention} uff! Kinda hard one!")

    def generate_embed(self) -> discord.Embed:
        return discord.Embed(
            color=self.bot.color, timestamp=self.message.created_at
        ).set_footer(
            text=f"{self.player}", icon_url=self.player.display_avatar.url
        ).add_field(
            name=f"Q. {self.q_n}",
            value=self.q
        )
    ...
    async def yes_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await self.message.edit(content=self.message.author.mention, embed=self.generate_embed(), view=self)
    ...
    async def quit_game(self, interaction: discord.Inteaction, button: discord.ui.Button):
        self.stop()
